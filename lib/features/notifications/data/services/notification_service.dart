...
class NotificationService {
  ...
  Future<void> initLocalNotifications() async {
    ...
    try {
      ...
      // Request notification permission (but not exact alarm permission)
  ...
  Future<void> showNotification({required RemoteMessage message}) async {
    try {
      ...
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString(_pendingNotificationKey, json.encode(message.data));
      ...
      if (currentUserId != null) {
        ...
        if (response.statusCode != 200) {
          log('Error saving notification: ${response.body}');
        }
      } else {
  ...
  Future<void> scheduleReminderNotification(ReminderModel reminder) async {
    try {
      ...
      if (filteredTips.isEmpty) {
        log('No tips found for reminder ${reminder.id}, category: ${reminder.categoryId}, type: ${reminder.type}');
        return;
      }
      ...
      final notificationId = Uuid().v4().hashCode.abs();
        ...
        if (navigator != null) {
          navigator.pushNamed(
            ...
            arguments: {
              ...
              'allHealthTips': type == 'tip',
              'allQuotes': type == 'quote',
            },
          );
          log('Navigated to TipsDetailScreen with tipId: $tipId');
          final prefs = await SharedPreferences.getInstance();

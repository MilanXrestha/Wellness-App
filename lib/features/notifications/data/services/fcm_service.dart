...
class FCMServices {
  ...
  Future<void> updateFcmTokenDirectly() async {
    try {
      log('Starting direct Firestore update of FCM token');

      // Get current user
      final user = AuthService().getCurrentUser();
      if (user == null) {
        log('No user signed in, skipping direct FCM token update');
        return;
      }
      ...
      if (token == null || token.isEmpty) {
        log('No FCM token available to update');
        return;
      }
      log('Got FCM token: ${token.substring(0, 10)}...');
      ...
      log('FCM token directly updated in Firestore for user ${user.uid}');
    } catch (e, stackTrace) {
      log('Error directly updating FCM token in Firestore: $e', stackTrace: stackTrace);
    }
  }
}
